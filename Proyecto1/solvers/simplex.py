...
class SolucionadorSimplex:
    """
    Implementación del Método Simplex tabular de Dos Fases.
    """
    ...
    def ejecutar_algoritmo_simplex(self):
        ...
        # Check if artificial variables exist (they start with 'A')
        ...
        if indices_artificiales:
            ...
            # Configurar fila W para Fase 1 (maximizar -W)
            ...
            # Hacer ceros los coeficientes de las variables artificiales (ya que son básicas)
            for i in range(self.cantidad_restricciones):
            ...
            # Configurar fila Z original
            if self.problema.tipo_objetivo == 'max':
                fila_z = -vector_z
            else:
                fila_z = vector_z # max -Z
                
            self.matriz_tabla_simplex[-1, :total_vars] = fila_z
            self.matriz_tabla_simplex[-1, -1] = 0.0
            
            # Restaurar consistencia: hacer 0 los reducidos de las variables básicas
            for i in range(self.cantidad_restricciones):
    ...
    def _imprimir_estado_tabla_simplex(self, numeracion_iteracion, nombre_fase="Iteración"):
        ...
        for indice_fila in range(self.matriz_tabla_simplex.shape[0]):
            texto_fila_numerica = "".join([f"{valor:8.2f}" for valor in self.matriz_tabla_simplex[indice_fila, :]])
            
            if indice_fila == self.matriz_tabla_simplex.shape[0] - 1:
                self._log("-" * len(texto_cabecera_columnas))
                self._log(texto_fila_numerica + "  (Fila Z/W)")
            else:
